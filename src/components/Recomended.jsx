...
const RecomendedCars = () => {
  ...
  const filteredCars = showAllCars
    ? popularCarsData
    : popularCarsData.slice(0, 8);
  const totalCars = popularCarsData.length;
  ...
  return (
    <div className="bg-[#F6F7F9]">
      ...
      <div className="flex items-center pb-16">
        <button
          className="text-[#FFF] text-base font-semibold bg-primary py-2 px-7 flex mx-auto rounded-[4px] cursor-pointer"
          onClick={() => setShowAllCars(!showAllCars)}
        >
          Show more Cars
        </button>
        <span className="text-sm text-[#90A3BF] px-10 font-normal">
          {totalCars} Cars
        </span>{" "}
      </div>
    </div>
